...
public class OldCalculator {
    ...
    public static void main(String[] args) {
        ...
        oldCalculator.setCurrentNum(new BigDecimal("3"));
        oldCalculator.setCurOperator("+");
        oldCalculator.setCurrentNum(new BigDecimal("3"));
        oldCalculator.calNum();
        oldCalculator.setCurOperator("*");
        oldCalculator.setCurrentNum(new BigDecimal("4"));
        oldCalculator.calNum();
        oldCalculator.undo();
        oldCalculator.redo();
        oldCalculator.setCurOperator("*");
        oldCalculator.setCurrentNum(new BigDecimal("5"));
        oldCalculator.calNum();
        oldCalculator.undo();
        oldCalculator.setCurOperator("+");
        oldCalculator.setCurrentNum(new BigDecimal("5"));
        ...
        oldCalculator.undo();
        oldCalculator.redo();
        List<String> commands = oldCalculator.getCalCommands();
        List<BigDecimal> totals = oldCalculator.getCalTotals();
        List<BigDecimal> numbers = oldCalculator.getCalNumbers();
        System.out.println("运行指令："+ commands.toString());
        System.out.println("计算值："+ totals.toString());
        System.out.println("计算数："+ numbers.toString());
        System.out.println("可回退步骤："+ oldCalculator.lastStepIndex);
    }
